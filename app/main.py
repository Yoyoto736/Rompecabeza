...
if 'soluciones' in st.session_state and st.session_state.soluciones:
    ...
    # Renderizado con estilo extraído (Flexbox + CSS)
    for f in range(8):
        cols = st.columns(8)
        for c in range(8):
            val_celda = matriz_visual[f][c]
            bg_color = PALETA_COLORES.get(val_celda, "#FFF")
            
            if val_celda == 0:
                contenido_html = "<span style='font-size: 20px;'>✨</span>"
                border_style = "border: 2px dashed #E74C3C;"
            else:
                contenido_html = f"<b style='color: white; font-weight: bold;'>{val_celda}</b>" if val_celda > 0 else ""
    ...
    with st.expander("Ver orden de ensamble paso a paso"):
        for paso_num, paso in enumerate(soluciones[idx], 1):
            st.write(
                f"🔹 **Pieza {paso['pieza']}**: Colocar en la esquina superior izquierda de su caja en la coordenada de la cuadrícula: Fila **{paso['coords'][0] + 1}**, Columna **{paso['coords'][1] + 1}**."
            )